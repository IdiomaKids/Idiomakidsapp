...
//Hacemos la query para seleccionar todos los datos que necesitamos
  $sql = "SELECT id_player, name, id_user, birthday, avatar FROM players WHERE id_user = $iduser";
  $stmt = $conn->prepare($sql);
  $stmt->bindParam(':id_user', $_SESSION['id_user']);
  $stmt->execute();
//Esta query es la que utilizamos para ver cuantos usuarios tiene el tutor ascociados
...
//Creamos el foreach que recorre todos los datos que hemos conseguido en la query pintandolos de manera visual
//Pintamos un usuario por cada resultado
...
//Con esta comprobación vemos si es uno el resultado que nos ha dado la query, y si es uno, no permite borrar al usuario
//Si no si que si nos dejaria borrar al usuario
if ($total == 1) {
